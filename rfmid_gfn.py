...
from data.RFMiDDataset import RFMIDImageDataset
...
opt = Options(num_epochs=100, num_classes=45)
model = ViT_GFN(opt=opt, num_classes=45)

preprocess = models.ViT_B_16_Weights.DEFAULT.transforms(antialias=True)

training_images_path = "rfmid/images/training"
training_annotations = "rfmid/ground_truths/training.csv"
train_dataset = RFMIDImageDataset(training_images_path, training_annotations, preprocess)
train_dataloader = DataLoader(train_dataset, batch_size=64, shuffle=False)
print(f"Loaded training dataset - {len(train_dataset)}")

validation_images_path = "rfmid/images/validation"
validation_annotations = "rfmid/ground_truths/validation.csv"
val_dataset = RFMIDImageDataset(validation_images_path, validation_annotations, preprocess)
val_dataloader = DataLoader(val_dataset, batch_size=64, shuffle=False)
print(f"Loaded validation dataset - {len(val_dataset)}")
