...
class ResNet_GFN(nn.Module):
    ...
    def __init__(
        ...
        if opt.use_pretrained:
            ...
            )  # the pretrained version has 1000 classes
        ...
        maskgenerator_input_shapes = [
            ...
            (512, 4, 4),
            (512, 4, 4),
        ]  # only apply dropout out on the last two blocks of resent18
        ...
        self.p_zx_mask_generators = construct_conditional_mask_generators(
            ...
            layer_dims=[
                DIMs[0] * DIMs[1] * DIMs[2] for DIMs in maskgenerator_input_shapes
            ],
        ...
        self.q_z_mask_generators = construct_unconditional_mask_generators(
            layer_dims=[x[0] for x in maskgenerator_input_shapes], hiddens=hiddens
        ).to(
            device
        )  # q(z)
        ...
        self.LogZ_unconditional = nn.Parameter(
            torch.tensor(0.0)
        )  # paritization function when the GFN does not condition on any input
        ...
        self.taskmodel_scheduler = torch.optim.lr_scheduler.MultiStepLR(
            self.taskmodel_optimizer, milestones=self.opt.schedule_milestone, gamma=0.1
        )
                ...
                if layer_idx == 0:
                    ...
                    if (EPSILON < self.random_chance) and (self.training):
                        ...
                        qz_mask_l = self.q_z_mask_generators[layer_idx](
                            torch.zeros(batch_size, 784).to(device), temperature
                        )  # 784 is an arbitary number here

                    qz_p_l = self.q_z_mask_generators[layer_idx].prob(
                        torch.zeros(batch_size, 784).to(device), qz_mask_l
                    )
                else:
        ...
        if self.opt.BNN:
            ...
            for layer in [
                ...
                for blockid in range(
                    2
                ):  ##number block per layer changes over architecture
                    identity = x
                    ...
                    print(f"Output shape that I get is: {out.shape}")
                        ...
                        if "bottomup" in mask:
                            if self.training:
                                ...
                                if layer_idx == 0:
                                    ...
                                    qzxy_p_l = self.q_zxy_mask_generators[
                                        layer_idx
                                    ].prob(
                                        torch.zeros(batch_size, out.shape[1]).to(
                                            device
                                ...
                                if layer_idx == 0:
                                    m_conditional_l = self.p_zx_mask_generators[
                                        layer_idx
...
class RandomMaskGenerator(nn.Module):
    ...
    def forward(self, x):
        return torch.bernoulli((1.0 - self.dropout_rate) * torch.ones(x.shape))
...
class multiMLPMaskGenerator(nn.Module):
    ...
    def forward(self, x1, x2, x3, T=1.0):
        probs_sampled = self._dist(
            x1, x2, x3, T
        )  # sample the mask using tempered policy
